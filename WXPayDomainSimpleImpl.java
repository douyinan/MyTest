...
public class WXPayDomainSimpleImpl implements IWXPayDomain {
	...
	public synchronized void report(final String domain, long elapsedTimeMillis, final Exception ex) {
		...
		if (ex == null) { // success
			if (info.succCount >= Variable.POSITION_2) { // continue succ, clear
															// error count
	...
	static class DomainStatics {
		...
		boolean isGood() {
			return connectTimeoutCount <= Variable.POSITION_2 && dnsErrorCount <= Variable.POSITION_2;
		}

		int badCount() {
			return connectTimeoutCount + dnsErrorCount * Variable.POSITION_5 + otherErrorCount / Variable.POSITION_4;
		}
	}

	private final int MIN_SWITCH_PRIMARY_MSEC = Variable.POSITION_3 * Variable.POSITION_60 * Variable.POSITION_1000; // 3
																														// minutes
